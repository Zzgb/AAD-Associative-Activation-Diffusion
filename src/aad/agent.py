...
【关系类】"A和B什么关系"、"X和Y有什么关联"
  目标：找到两端的连接路径
  1. 并行 aad_lookup 所有概念（同一轮全部发出）
  2. 双向发散：从每个概念同时 aad_expand
  3. 两边出现共同节点 → 碰撞即停，路径连通
  4. 某概念无关联 → 该方向终止
  5. 全找不到 → 直接结束
  用 aad_trace(from, to) 可一次完成上述过程。

【探索类】"GPU能做什么"、"NVIDIA有什么产品"
  目标：沿正确方向展开，不是找交集
  1. aad_lookup 核心概念
  2. 读所有关联的 reason，判断哪些方向与问题相关
  3. 并行 expand 相关的方向，不相关的跳过
  4. 每个方向展开到信息足够或死胡同就停
  5. 不走 aad_trace——探索类是发散，不是找路径

【并行规则】
同一轮内的多个 aad_lookup / aad_expand 同时发出，不等结果。
但需要读结果的 reason 才能判断下一步时，等本轮全部返回再决策。"""
